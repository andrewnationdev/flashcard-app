...
export default function ButtonsRowComponent(props: IButtonsRowComponent) {
    ...
    return <div className="flex gap-4">
        <button
            ...
            className="flex-1 cursor-pointer bg-slate-900 border border-red-900/50 text-red-400 py-4 rounded-2xl hover:bg-red-950/50 hover:scale-[1.02] transition-all active:scale-95"
        >
            <X className="mx-auto" />
        </button>
        <button
            onClick={() => { updateScore(props.currentCard.id, 1); addPoints(10); props.setFlipped(false); }}
            className="flex-1 cursor-pointer bg-slate-900 border border-green-900/50 text-green-400 py-4 rounded-2xl hover:bg-green-950/50 hover:scale-[1.02] transition-all active:scale-95"
        >
